...
from lts_gns.algorithms.simulators.mgn_simulator import MGNSimulator, mse
...
class MGN(AbstractAlgorithm):
    ...
    def _large_eval_step(self, use_validation_data: bool = True) -> ValueDict:
        ...
        if self.algorithm_config.evaluation.large.mse_ggns_style:
            ggns_mse_result = self._rollout_mse_ggns_style(task_list)
        ...
        for task_idx in task_indices:
            ...
            full_rollout_results[keys.SCALARS][keys.LARGE_EVAL][f"Task_{task_idx}_scalars"] = rollout_metrics[keys.SCALARS]
            ...
            with torch.no_grad():
                ...
                trajectory_metrics = self.simulator.evaluate_state(predicted_mesh_state=updated_mesh_state,
                                                                   reference_labels=eval_task.trajectory[
                                                                       -1].next_mesh_pos)
                full_metrics |= prefix_dict(to_numpy(trajectory_metrics), prefix=f"{recompute_edge_str}")
    ...
    def _k_step_evaluations(self, task_collection: TaskCollection) -> ValueDict:
        ...
        for first_step_only in [True, False]:
            for recompute_edges in recompute_edge_list:
                recompute_edge_str = f"recomputed_edges" if recompute_edges else "fixed_edges"
                for task_idx, current_task in tqdm(enumerate(task_collection),
                                                   desc="Large k-step evaluation",
                                                   total=len(task_collection)):
                    self._evaluate_k_step_task(current_task, metrics, max_k, multi_step_evaluations,
                                               recompute_edge_str, recompute_edges, task_idx,
                                               first_step_only=first_step_only)

        # gather the results by averaging over all tasks
        return_metrics = {key: np.mean(value) for key, value in metrics.items()}
        return return_metrics

    def _evaluate_k_step_task(self, eval_task, metrics, max_k, multi_step_evaluations,
            ...
            for k in range(1, max_k + 1):
                ...
                if k in multi_step_evaluations:  # gather metrics of the k-step prediction
                    ...
                    for key, value in evaluations.items():
                        ...
                        metrics[f"{dict_prefix}{k}-step {key}"].append(value)
