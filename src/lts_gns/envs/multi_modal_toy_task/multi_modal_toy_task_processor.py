...
from lts_gns.envs.multi_modal_toy_task.multi_modal_toy_task_util import load_raw_data, select_and_normalize_attributes, \
    build_data_dict, \
    build_graph
...
class MultiModalToyTaskProcessor(AbstractDataLoaderProcessor):
    ...
    def _load_raw_data(self, split: str) -> List[ValueDict]:
        if self.preprocess_config.save_load.version == "direct_start":
            return load_raw_data(self.preprocess_config.save_load.path_to_datasets,
                                 self.task_name + "_direct_start", split)
        else:
            return load_raw_data(self.preprocess_config.save_load.path_to_datasets,
    ...
    def _build_graph(self, data_dict: ValueDict) -> Data:
        return build_graph(data_dict, self.preprocess_config.task_properties_input_selection,
                           self.preprocess_config.use_vel_features)
    ...
    def get_integrate_predictions_fn(self) -> Callable[[torch.Tensor, Batch], Dict[str, torch.Tensor]]:
        return wrapped_partial(integrate_predictions, integration_order=1, d_t=0.1)
    ...
    def get_update_batch_fn(self) -> Callable[[Batch, Dict[str, torch.Tensor]], Batch]:
        def update_batch(batch: Batch, mesh_state: Dict[str, torch.Tensor]) -> Batch:
            ...
            if len(mesh_state[keys.POSITIONS].shape) == 2:
                ...
                new_velocities = mesh_state[keys.VELOCITIES]
                ...
                new_velocities = mesh_state[keys.VELOCITIES][0]
            else:
                raise ValueError("The Positions must have shape (num_samples, num_nodes, 2) or (num_nodes, 2)")
            batch.pos = new_positions
            if self.preprocess_config.use_vel_features:
                # update also the x features: x = [pos, vel, possible_task_properties]
                batch.x = torch.cat([batch.pos, new_velocities, batch.x[:, 4:]], dim=1)
            else:
                # x = [pos, possible_task_properties]
