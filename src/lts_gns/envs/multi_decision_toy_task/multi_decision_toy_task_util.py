...
def build_data_dict(task: ValueDict, timestep: int, world_to_model_normalizer: WorldToModelNormalizer) -> ValueDict:
    ...
    data_dict = {keys.MESH: torch.tensor(task[keys.MESH][timestep], dtype=torch.float32),
                 ...
                 "time_step": torch.tensor([timestep], dtype=torch.float32) / 100,  # normalize between 0 and 1
                 }

    # have the normalized velocities (e.g. in model space) as labels
    model_velocities = compute_model_velocities(data_dict, dt=1.0, world_to_model_normalizer=world_to_model_normalizer)
    data_dict[keys.LABEL] = model_velocities

    return data_dict


def build_graph(data_dict: ValueDict, task_properties_input_selection: str, use_pos_features: bool,
                use_time_features: bool) -> Data:
    """
    ...
    """
    # build nodes features (position and timestep options)
    pos = einops.rearrange(data_dict[keys.MESH], 'd_world -> 1 d_world')
    time_step = einops.rearrange(data_dict["time_step"], 'd_world -> 1 d_world')
    x = []
    if use_pos_features:
        x.append(pos)
    ...
    task_properties = torch.cat(
        [data_dict["goal_pos"].flatten(), data_dict["mass"], data_dict["k_p"], data_dict["t_d"]]).reshape(1, -1)
